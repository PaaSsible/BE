...
public class ChatRoomController {
    ...
    public ResponseEntity<ApiResponse<ChatRoomResponse>> getBoardChatRoom(@AuthenticationPrincipal UserJwtDto user,
                                                                          @PathVariable Long boardId) {
        ChatRoomResponse response = chatRoomService.getGroupChatRoom(user.getUserId(), boardId);
        return ResponseEntity.ok(ApiResponse.success(SuccessCode.OK, response));
    }

    @PostMapping("/direct")
    @Operation(summary = "1:1 채팅방 조회", description = "선택한 상대방과의 1:1 채팅방을 생성하거나 roomId를 조회합니다.")
    public ResponseEntity<ApiResponse<ChatRoomResponse>> getOrCreateDirectRoom(@AuthenticationPrincipal UserJwtDto user,
                                                                               @RequestBody DirectChatRequest request) {
        ChatRoomResponse response = chatRoomService.getOrCreateDirectChat(user.getUserId(), request);
        return ResponseEntity.ok(ApiResponse.success(SuccessCode.OK, response));
    }

    @PostMapping("/group/{boardId}/sub")
    @Operation(summary = "서브 채팅방 생성", description = "새로운 채팅방을 생성합니다.")
    public ResponseEntity<ApiResponse<Void>> createSubChat(@AuthenticationPrincipal UserJwtDto user,
                                                           @PathVariable Long boardId,
                                                           @RequestBody SubChatRequest request) {
        chatRoomService.createSubChat(user.getUserId(), boardId, request);
